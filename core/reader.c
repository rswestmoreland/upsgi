...
int uwsgi_request_body_seek_do(struct wsgi_request *wsgi_req, off_t pos, int whence) {
	off_t new_pos = 0;

	if (wsgi_req->post_file) {
		if (fseek(wsgi_req->post_file, pos, whence)) {
			uwsgi_req_error("uwsgi_request_body_seek_do()/fseek()");
			wsgi_req->read_errors++;
			return -1;
		}
		new_pos = ftell(wsgi_req->post_file);
		if (new_pos < 0) {
			uwsgi_req_error("uwsgi_request_body_seek_do()/ftell()");
			wsgi_req->read_errors++;
			return -1;
		}
...
void uwsgi_request_body_seek(struct wsgi_request *wsgi_req, off_t pos) {
	(void) uwsgi_request_body_seek_do(wsgi_req, pos, SEEK_SET);
}
...
char *uwsgi_request_body_read(struct wsgi_request *wsgi_req, ssize_t hint, ssize_t *rlen) {
	...
	if (wsgi_req->body_is_chunked) {
		...
		struct uwsgi_buffer *chunked = uwsgi_chunked_read_smart(wsgi_req, remains, 0);
		if (!chunked) {
			wsgi_req->read_errors++;
			*rlen = -1;
			return NULL;
...
int uwsgi_postbuffer_do_in_mem(struct wsgi_request *wsgi_req) {
        ...
        if (remains > uwsgi.post_buffering_bufsize) {
                ptr = malloc(remains);
                if (!ptr) {
                        uwsgi_error("uwsgi_postbuffer_do_in_mem()/malloc()");
                        wsgi_req->read_errors++;
                        return -1;
                }
...
int uwsgi_postbuffer_do_in_disk(struct wsgi_request *wsgi_req) {
        ...
        while (post_remains > 0) {
                ...
                size_t remains = UMIN(post_remains, uwsgi.post_buffering_bufsize);
