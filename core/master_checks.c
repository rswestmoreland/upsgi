...
void uwsgi_master_check_chain() {
	...
	if (uwsgi.chain_reload_delay > 0 && last_chain_curse > 0) {
		time_t now = uwsgi_now();
		if (now - last_chain_curse < uwsgi.chain_reload_delay) {
			if (now != last_pacing_check) {
				uwsgi_log_verbose("chain reload pacing delay active (%d sec between worker turnovers)\n", uwsgi.chain_reload_delay);
