...
static struct uwsgi_option uwsgi_base_options[] = {
	 ...
	 * - all upsgi compatibility-only flags route through uwsgi_opt_compat_noop()
	 */
	{"http-socket", required_argument, 0, "bind to the specified UNIX/TCP socket using HTTP protocol", uwsgi_opt_add_socket, "http", 0},
	{"http-socket-modifier1", required_argument, 0, "accepted for migration compatibility; no runtime effect in upsgi", uwsgi_opt_compat_noop, NULL, 0},
	{"http-socket-modifier2", required_argument, 0, "accepted for migration compatibility; no runtime effect in upsgi", uwsgi_opt_compat_noop, NULL, 0},
	{"http-modifier1", required_argument, 0, "accepted for migration compatibility; no runtime effect in upsgi", uwsgi_opt_compat_noop, NULL, 0},
	{"http-modifier2", required_argument, 0, "accepted for migration compatibility; no runtime effect in upsgi", uwsgi_opt_compat_noop, NULL, 0},
	...
	{"https-socket-modifier1", required_argument, 0, "accepted for migration compatibility; no runtime effect in upsgi", uwsgi_opt_compat_noop, NULL, 0},
	{"https-socket-modifier2", required_argument, 0, "accepted for migration compatibility; no runtime effect in upsgi", uwsgi_opt_compat_noop, NULL, 0},
#endif
	...
	{"chain-reload-delay", required_argument, 0, "wait the specified seconds between chain reload worker turnovers", uwsgi_opt_set_int, &uwsgi.chain_reload_delay, UWSGI_OPT_MASTER},
	...
	{"post-buffering", required_argument, 0, "set size in bytes after which request bodies spill to disk instead of staying fully in memory", uwsgi_opt_set_64bit, &uwsgi.post_buffering, 0},
	{"post-buffering-bufsize", required_argument, 0, "set read chunk buffer size used while post buffering; independent from the in-memory spill threshold", uwsgi_opt_set_64bit, &uwsgi.post_buffering_bufsize, 0},
	...
	{"log-drain-burst", required_argument, 0, "set the maximum number of log records drained per wake before yielding back to the event loop", uwsgi_opt_log_drain_burst, &uwsgi.log_drain_burst, 0},
	...
	{"log-x-forwarded-for", no_argument, 0, "log the client IP from X-Forwarded-For instead of REMOTE_ADDR", uwsgi_opt_true, &uwsgi.logging_options.log_x_forwarded_for, 0},
	...
	{"static-map", required_argument, 0, "map mountpoint to static directory (or file); baseline static serving path", uwsgi_opt_static_map, &uwsgi.static_maps, UWSGI_OPT_MIME},
	...
	{"cflags", no_argument, 0, "report core build CFLAGS (useful for building external plugins)", uwsgi_opt_cflags, NULL, UWSGI_OPT_IMMEDIATE},
	...
	{"config-py", no_argument, 0, "dump the upsgiconfig.py used for building the core (useful for building external plugins)", uwsgi_opt_config_py, NULL, UWSGI_OPT_IMMEDIATE},
	{"build-plugin", required_argument, 0, "build a plugin for the current binary", uwsgi_opt_build_plugin, NULL, UWSGI_OPT_IMMEDIATE},
	{"version", no_argument, 0, "print upsgi version", uwsgi_opt_print, UWSGI_VERSION, 0},
...
check:
	...
	while (poll(&pfd, 1, 0) > 0) {
		if (!uwsgi_master_log_drain(uwsgi.log_drain_burst)) {
			break;
		}
	}
}
...
unsafe:
	...
	uwsgi_validate_runtime_tunables();
...
int uwsgi_run() {
	...
	uwsgi_map_sockets();
	uwsgi_configure_worker_accept_mode();
...
void uwsgi_opt_log_drain_burst(char *opt, char *value, void *key) {
	int *ptr = (int *) key;
	if (value) {
		char *endptr;
	...
	if (*ptr < 1) {
		uwsgi_log("invalid log-drain-burst %d, clamping to 1\n", *ptr);
		*ptr = 1;
	}
	else if (*ptr > 1024) {
		uwsgi_log("excessive log-drain-burst %d, clamping to 1024\n", *ptr);
		*ptr = 1024;
	}
}
 ...
 */
void uwsgi_opt_compat_noop(char *opt, char *foo, void *bar) {
	(void) opt;
	(void) foo;
	(void) bar;
}
