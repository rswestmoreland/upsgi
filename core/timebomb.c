...
static void *time_bomb(void *arg) {
	...
	upsgi_log_verbose("*** BOOOOOOM ***\n");
	exit(tb->exit_code);
	
}

void upsgi_time_bomb(int timeout, int exit_code) {

	pthread_t time_bomb_thread;

	struct time_bomb *tb = upsgi_malloc(sizeof(struct time_bomb));
	tb->timeout = timeout;
	tb->exit_code = exit_code;
	
	if (pthread_create(&time_bomb_thread, NULL, time_bomb, (void *) tb)) {
        	upsgi_error("pthread_create()");
                upsgi_log("unable to setup the time bomb, goodbye\n");
		exit(exit_code);
	}
        else {
        	upsgi_log_verbose("Fire in the hole !!! (%d seconds to detonation)\n", timeout);
        }
