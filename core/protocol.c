...
static int uwsgi_proto_check_14(struct wsgi_request *wsgi_req, char *key, char *buf, uint16_t len) {
	...
	if (!uwsgi_proto_key("CONTENT_LENGTH", 14)) {
		...
		if (get_content_length(buf, len, &parsed_content_length)) {
			uwsgi_log("Invalid CONTENT_LENGTH. skip.\n");
			return -1;
		}
		if (wsgi_req->body_is_chunked) {
			uwsgi_log("Invalid request: chunked Transfer-Encoding with Content-Length. skip.\n");
			return -1;
		}
...
nextcs:
	 ...
	 * delegated to uwsgi_file_serve() in core/static.c.
	 */
	// check static-map
