...
int upsgi_regexp_build(char *re, upsgi_pcre ** pattern) {
	...
	*pattern = upsgi_malloc(sizeof(upsgi_pcre));
	(*pattern)->p = pcre_compile((const char *) re, 0, &errstr, &erroff, NULL);
#endif
#ifdef UPSGI_PCRE2
	if (!(*pattern)) {
		upsgi_log("pcre error: code %d at offset %d\n", errnbr, erroff);
#else
	if (!((*pattern)->p)) {
		upsgi_log("pcre error: %s at offset %d\n", errstr, erroff);
#endif
		return -1;
	}
...
int upsgi_regexp_match(upsgi_pcre *pattern, const char *subject, int length) {
#ifdef UPSGI_PCRE2
	return upsgi_regexp_match_ovec(pattern, subject, length, NULL, 0);
#else
	return pcre_exec((const pcre *) pattern->p, (const pcre_extra *) pattern->extra, subject, length, 0, 0, NULL, 0);
#endif
}

int upsgi_regexp_match_ovec(upsgi_pcre *pattern, const char *subject, int length, int *ovec, int n) {
	 ...
	 * capturing subpatterns (INFO_CAPTURECOUNT), from upsgi_regexp_ovector(),
	 * as matching pairs, plus room for the first pair.
	 */
	if (n > 0) {
		// copy pcre2 output vector to upsgi output vector
...
int upsgi_regexp_ovector(const upsgi_pcre *pattern) {
...
char *upsgi_regexp_apply_ovec(char *src, int src_n, char *dst, int dst_n, int *ovector, int n) {
	...
	char *res = upsgi_malloc(dst_n + (src_n * dollars) + 1);
	char *ptr = res;
