...
python3 "$asset_common/generate_payloads.py" --out "$work_dir/r3_payload.json" --size 262144
python3 "$asset_common/receiver_client.py" --host 127.0.0.1 --port "$port" --payload "$work_dir/r3_payload.json" --content-type application/json --expect-status 202
[[ -s "$queue_file" ]]
