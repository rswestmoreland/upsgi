...
python3 "$asset_common/generate_payloads.py" --out "$work_dir/r4_payload.json" --size 32768
python3 "$asset_common/slow_uploader_client.py" --host 127.0.0.1 --port "$port" --payload "$work_dir/r4_payload.json" --content-type application/json --chunk-size 4096 --delay-ms 10 > "$artifact_dir/r4_slow.out"
grep -q "202" "$artifact_dir/r4_slow.out"
[[ -s "$queue_file" ]]
