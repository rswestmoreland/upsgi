...
python3 "$asset_common/generate_payloads.py" --out "$work_dir/r2_payload.gz" --size 32768 --compress
python3 "$asset_common/receiver_client.py" --host 127.0.0.1 --port "$port" --payload "$work_dir/r2_payload.gz" --content-type application/octet-stream --expect-status 202
[[ -s "$queue_file" ]]
