...
config_yaml="$work_dir/r5.yaml"
log_file="$artifact_dir/r5.log"
app="$asset_common/receiver_app.psgi"
queue_file="$artifact_dir/r5.queue"
...
export PERF_R_APPEND_DELAY_MS=5
python3 "$asset_common/generate_payloads.py" --out "$work_dir/r5_payload.json" --size 16384
python3 "$asset_common/receiver_client.py" --host 127.0.0.1 --port "$port" --payload "$work_dir/r5_payload.json" --content-type application/json --expect-status 202
[[ -s "$queue_file" ]]
