...
PORT=${PORT:-18084}
LOG_FILE=${LOG_FILE:-$ASSET_DIR/a4_smoke.log}
CFG_FILE=${CFG_FILE:-$ASSET_DIR/a4_smoke.yaml}
...
out = src % {
    'port': $PORT,
    'app': str(Path(r"$ASSET_DIR/app_download_mix.psgi")),
...
sleep 1
python3 "$ASSET_DIR/download_mix_client.py" --port "$PORT" --sequence "$ASSET_DIR/download_matrix.json"
