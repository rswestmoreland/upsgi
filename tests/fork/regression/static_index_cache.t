use strict;
use warnings;

use File::Spec;
use FindBin;
use IO::Socket::INET;
use JSON::PP qw(decode_json);
use Test::More;

use lib File::Spec->catdir($FindBin::Bin, '..', 'lib');
use UpSGITest qw(append_yaml_options build_artifact_dir default_binary fixture_app fixture_static_root http_get pick_port render_profile start_server stop_server wait_http);

sub fetch_stats {
    my ($port) = @_;
    my $sock = IO::Socket::INET->new(
        PeerAddr => '127.0.0.1',
        PeerPort => $port,
        Proto => 'tcp',
        Timeout => 5,
    ) or die "unable to connect to stats socket on port $port: $!\n";

    my $json = '';
    while (1) {
        my $buf = '';
        my $read = sysread($sock, $buf, 4096);
        last if !defined($read) || $read == 0;
        $json .= $buf;
    }
    close $sock;

    return decode_json($json);
}

sub worker_core {
    my ($stats) = @_;
    return $stats->{workers}[0]{cores}[0];
}

my $binary = default_binary();
my $artifact_dir = build_artifact_dir('static_index_cache');
my $config_yaml = File::Spec->catfile($artifact_dir, 'baseline.yaml');
my $server_log = File::Spec->catfile($artifact_dir, 'server.log');
my $port = pick_port(44);
my $stats_port = pick_port(45);

render_profile(
    profile => 'baseline',
    output_yaml => $config_yaml,
    app => fixture_app('app_simple.psgi'),
    static_root => fixture_static_root(),
    log_file => $server_log,
    port => $port,
);

append_yaml_options(
    $config_yaml,
    "stats = 127.0.0.1:$stats_port\n",
    "cache2 = name=staticpaths,items=64,blocksize=4096\n",
    "static-cache-paths = 60\n",
    "static-cache-paths-name = staticpaths\n",
    "static-index = index.js\n",
);

start_server(
    binary => $binary,
    config_yaml => $config_yaml,
    artifact_dir => $artifact_dir,
);

END {
    stop_server($artifact_dir) if $artifact_dir;
}

ok(wait_http(host => '127.0.0.1', port => $port, path => '/assets/nested/'), 'static index cache profile becomes reachable');

my $warm = http_get(host => '127.0.0.1', port => $port, path => '/assets/nested/');
is($warm->{status}, 200, 'warm request for directory index returns 200');
like($warm->{content}, qr/upsgi static nested ok/, 'warm request returns nested index body');

my $before_stats = fetch_stats($stats_port);
my $before_core = worker_core($before_stats);

for (1 .. 3) {
    my $resp = http_get(host => '127.0.0.1', port => $port, path => '/assets/nested/');
    is($resp->{status}, 200, 'cached directory index hit returns 200');
    like($resp->{content}, qr/upsgi static nested ok/, 'cached directory index body is returned');
}

my $after_stats = fetch_stats($stats_port);
my $after_core = worker_core($after_stats);

cmp_ok(
    $after_core->{static_path_cache_hits},
    '>=',
    $before_core->{static_path_cache_hits} + 3,
    'directory index cache hits increase after the warm request',
);
is(
    $after_core->{static_realpath_calls},
    $before_core->{static_realpath_calls},
    'cached directory index hits avoid additional realpath calls',
);
is(
    $after_core->{static_index_checks},
    $before_core->{static_index_checks},
    'cached directory index hits avoid additional index probes',
);
cmp_ok(
    $after_core->{static_open_calls},
    '>=',
    $before_core->{static_open_calls} + 3,
    'cached directory index hits increment static open calls on the fast path',
);
is(
    $after_core->{static_stat_calls},
    $before_core->{static_stat_calls},
    'cached directory index hits avoid extra static stat calls on the fast path',
);

done_testing();
