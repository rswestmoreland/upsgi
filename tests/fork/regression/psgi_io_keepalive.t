...
my $artifact_dir = build_artifact_dir('psgi_io_keepalive');
my $config_yaml = File::Spec->catfile($artifact_dir, 'http11_psgix_io.yaml');
...
rewrite_http11_profile_with_psgix_io($config_yaml);
...
ok(wait_http(host => '127.0.0.1', port => $port, path => '/ready'), 'http11 psgix.io profile becomes reachable');

my $sock = open_socket($port);
print {$sock} "GET /one HTTP/1.1\r\nHost: 127.0.0.1:$port\r\n\r\n";
my $first = read_http_response($sock);
like($first->{status_line}, qr{\AHTTP/1\.[01] 200\b}, 'first keepalive response with psgix.io enabled returns 200');
is($first->{body}, "method=GET\npath=/one\n", 'first keepalive response body matches request path');
ok(!exists $first->{headers}{connection}, 'first keepalive response with psgix.io enabled does not force Connection close');

print {$sock} "GET /two HTTP/1.1\r\nHost: 127.0.0.1:$port\r\nConnection: close\r\n\r\n";
my $second = read_http_response($sock);
like($second->{status_line}, qr{\AHTTP/1\.[01] 200\b}, 'second keepalive response with psgix.io enabled returns 200');
...
is($eof, '', 'socket closes only after the explicit Connection close request');
close $sock;
