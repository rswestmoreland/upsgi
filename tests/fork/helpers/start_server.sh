...
usage() {
    echo "usage: $0 <binary> <config_ini> <artifact_dir>" >&2
    exit 2
}
...
: > "$server_log"
rm -f "$pid_file"
printf '%q --ini %q --pidfile %q\n' "$binary" "$config_ini" "$pid_file" > "$launch_file"
{
    echo "UPSIG_BINARY=$binary"
    echo "UPSIG_CONFIG=$config_ini"
    echo "UPSIG_ARTIFACT_DIR=$artifact_dir"
    echo "UPSIG_SERVER_LOG=$server_log"
    echo "UPSIG_PIDFILE=$pid_file"
} > "$meta_file"

"$binary" --ini "$config_ini" --pidfile "$pid_file" >>"$server_log" 2>&1 &
launch_pid=$!
