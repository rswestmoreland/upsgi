...
#define thunder_lock if (!uwsgi.is_et) {\
                        if (uwsgi.use_thunder_lock && !uwsgi.skip_thunder_lock) {\
...
void uwsgi_opt_compat_noop(char *, char *, void *);
...
int uwsgi_master_log_drain(int);
int uwsgi_master_req_log_drain(int);
...
void uwsgi_configure_worker_accept_mode(void);
...
int uwsgi_request_body_seek_do(struct wsgi_request *, off_t, int);
